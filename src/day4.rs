...
pub fn run_day4(path: &str) -> u32 {
    let guard_sleep_schedule: HashMap<u32, Vec<i32>> = day4_get_schedule(path);

    let (sleepy_guard_id, minutes) = guard_sleep_schedule
        .into_iter()
    ...
    let sleepy_minute = minutes
        .into_iter()
        .enumerate()
        .max_by_key(|(_index, counts)| *counts)
        .unwrap()
        .0;
...
pub fn run_day4_part2(path: &str) -> u32 {
    ...
    let (sleepy_guard_id, minutes) = guard_sleep_schedule
        .into_iter()
        .max_by_key(|(_id, minutes)| *minutes.iter().max().unwrap())
        .unwrap();
    ...
    for line in sorted_schedule.iter() {
        ...
        match guard_category {
            ...
            GuardEvent::WakesUp(minute) => {
                let sleeps = guard_sleep_schedule.get_mut(&guard_id.unwrap()).unwrap();
...
fn get_guard_event(string: &str) -> GuardEvent {
    if string.contains("falls asleep") {
        ...
        let caps = claims_regex.captures(string).unwrap();
...
#[cfg(test)]
mod tests {
    use crate::day4::{get_guard_event, run_day4, run_day4_part2, GuardEvent};
